...
import { UploadIcon, MagicWandIcon, PaletteIcon, SunIcon, WhiskIcon } from './icons';

interface StartScreenProps {
  onFileSelect: (files: FileList | null) => void;
  onSwitchToWhisk: () => void;
}

const StartScreen: React.FC<StartScreenProps> = ({ onFileSelect, onSwitchToWhisk }) => {
    ...
    >
      <div className="flex flex-col items-center gap-6 animate-fade-in">
        ...
        <div className="mt-6 flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <button onClick={onSwitchToWhisk} className="relative inline-flex items-center justify-center px-8 py-4 text-lg font-bold text-gray-200 bg-white/10 rounded-full cursor-pointer group hover:bg-white/20 transition-colors">
                <WhiskIcon className="w-6 h-6 mr-3 text-cyan-400 transition-transform duration-300 group-hover:scale-110" />
                Create with Whisk AI
            </button>
        </div>
        <p className="text-sm text-gray-500 -mt-2">or drag and drop files to start editing</p>
